# from http://search.cpan.org/src/RGARCIA/perl-5.10.0/lib/Benchmark.pm
# with minor modifications: s/our/use vars/ s/Benmark::import// s/Benmark/MyBenchmark/ s/pod//;
...
use vars qw($Debug $Min_Count $Min_CPU $Default_Format $Default_Style
...
sub runloop {
    ...
    my $tbase = MyBenchmark->new(0)->[1];
    while ( ( $t0 = MyBenchmark->new(0) )->[1] == $tbase ) {} ;
    $subref->();
    $t1 = MyBenchmark->new($n);
    $td = &timediff($t1, $t0);
